...
@customElement("ha-multi-entity-picker")
export class HaEntityPicker extends LitElement {
    ...
    (
      ...
      if (includeDeviceClasses) {
        states = states.filter(
          (stateObj) =>
            // We always want to include the entities of the current values
            this._value.includes(stateObj.entity_id) ||
      ...
      if (entityFilter) {
        states = states.filter(
          (stateObj) =>
            // We always want to include the entities of the current values
            this._value.includes(stateObj.entity_id) || entityFilter!(stateObj)
        );
      }
  ...
  private _valueChanged(ev: PolymerChangedEvent<string>) {
    ...
    if (this._value.includes(newValue)) {
      const index = currentValue.indexOf(newValue, 0);
      if (index > -1) {
  ...
  private _setValue(value: string[]) {
    this.value = value;
    setTimeout(() => {
      // This currently leads into an infinite loop
...
declare global {
  interface HTMLElementTagNameMap {
    "ha-multi-entity-picker": HaEntityPicker;
  }
}
